...
import src.terracraft.items.ModItemKey;
import src.terracraft.tileEntities.ModTileEntityModChest;

import cpw.mods.fml.relauncher.Side;
import cpw.mods.fml.relauncher.SideOnly;

public class ModBlockModChestLocked extends ModBlockModChest implements ITileEntityProvider
{
    private final Random random = new Random();
    private static final String __OBFID = "CL_00000214";
    private String chestType;

    public ModBlockModChestLocked(String name)
    {
        super(name);
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer entityPlayer, int p_149727_6_, float p_149727_7_, float p_149727_8_, float p_149727_9_)
    {
    	...
    	{
    		boolean isGoldKey = entityPlayer.inventory.getCurrentItem().getItem().equals(ModItem.keyGold);
    		boolean isShadowKey = entityPlayer.inventory.getCurrentItem().getItem().equals(ModItem.keyShadow);
        	if(isGoldKey && this.chestType == "goldLocked")
        	{
    	    	entityPlayer.inventory.decrStackSize(entityPlayer.inventory.currentItem, 1);
    	    	world.setBlock(x, y, z, ModBlock.chestGold);
    	    	return true;
    	    }
        	else if(isShadowKey && this.chestType == "shadowLocked")
        	{
    		    entityPlayer.inventory.decrStackSize(entityPlayer.inventory.currentItem, 1);
    		    world.setBlock(x, y, z, ModBlock.chestShadow);
    		    return true;
    		}
    	}
