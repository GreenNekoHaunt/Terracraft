...
public class ModBlockPearlsand extends ModBlockFalling
{
    public ModBlockPearlsand(String name, Material material)
    {
        super(name, material);
...
/*		>> IF WORLD IS HARD MODE <<
                    ...
                    {
                        par1World.setBlock(i1, j1, k1, ModBlock.pearlstone.blockID);
                    }
                    if (par1World.getBlockId(i1, j1, k1) == Block.sand.blockID
                        && par1World.getBlockLightOpacity(i1, j1 + 1, k1) <= 2)
                    {
                        par1World.setBlock(i1, j1, k1, ModBlock.pearlsand.blockID);
                    }
		>> IF END <<
*/
                    if (par1World.getBlock(i1, j1, k1) == Blocks.grass
                        && par1World.getBlockLightOpacity(i1, j1 + 1, k1) <= 2)
                    {
                        par1World.setBlock(i1, j1, k1, ModBlock.grassHallowed);
                    }
                }
            }
        }
